...
static double mapx(int i, int j){
    ...
    return (5*4-Math.sin((i+4)*slice)*5*4)*width_multiplier;
}
static double mapy(int i, int j){
    return j*5*height_multiplier;
}
static double mapz(int i, int j){
    double slice = ((Math.PI*2)/4)/8*2;

    return Math.cos((i+4)*slice)*5*4*width_multiplier;
}
...
public static double whrat;
public static boolean situation_a;
public static double width_multiplier = 1;
public static double height_multiplier = 1;
public static int[] data;
public static ByteBuffer main(String[] args){
    width_multiplier = 1;
    height_multiplier = 1;
    if(height > width){
        width_multiplier = width * 1.0 / height;
    }
    if(height < width){
        height_multiplier = height * 1.0 / width;
    }
