...
layout (location = 11) flat out int mode;
...
void main()
{
    ...
    //     pos.z*=10;
    // }
    { // ok well at least this works here
        float idkval = 10;
        if(mode >= 2000 && mode <= 2003){
            pos.x += c8.x;
            pos.y += c8.y;
        }
        if(mode == 2003){
    ...
    if(mode == 2000 || mode == 2001 || mode == 2002 || mode == 2003){
