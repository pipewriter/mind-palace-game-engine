...
layout (location = 11) flat in int m;
...
uniform int blinklevel; // out of a million
...
void main(){
    ...
    if(about(m,8)==1){
        ...
        if(stx>0.98 || stx < 0.02 || sty > 0.98 || sty < 0.02){
            ...
            float oga = colorFromPic.a;
            vec4 c2 = vec4(1) - colorFromPic;
            //float ee = mod(distance(st,vec2(0.5)) + clock/1000.0, 1.0);
            float dis = distance(st,vec2(0.5));
            float ee = sin(dis*5 - 6.28*clock/1000.0)/2+0.5;
            colorFromPic = mix(colorFromPic, c2, ee);
            colorFromPic.a = oga;
        }
    }else
    if(about(m,99)==1){
    ...
    // float blinklevel2 = ((clock%1000)/1000.0);
    // blinklevel2 = ((blinklevel%1000)/1000.0);
    float blinklevel2 = abs(blinklevel-1000)/1000.0;
    // if(m < 1000 && abs(tp.y)+ abs(tp.x) > blinklevel2){
    if(m < 1000 && sqrt(tp.y*tp.y+tp.x*tp.x) > blinklevel2){
        color.rgb = vec3(0);
        return;
    }else if(m > 1000 && blinklevel < 2000){
        //hide top overlay
        color.rgb = vec3(0);
        return;
